...
@section('content')
    ...
    <div class="flex items-center gap-2">
        <a href="{{ route('edit_pengaduan', $com->id) }}" class="bg-blue-600 hover:bg-blue-700 text-white text-xs px-3 py-1.5 rounded-md">Edit</a>
        
        <form id="delete-form-{{ $com->id }}" action="{{ route('tabel_user.destroy', $com->id) }}" method="POST">
            @csrf
            @method('DELETE')
            <button type="button" 
...
<script>
    function confirmDelete(id) {
        Swal.fire({
            ...
            confirmButtonColor: '#ff0000a7',
            cancelButtonColor: '#6b7280',
            confirmButtonText: 'Hapus Pengaduan ini',
